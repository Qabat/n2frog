...
while ((finalGError > errorTolerance) && (finalIterations < maxIterations))
    ...
    if (useBootstrap == 1)
        oldEFr = retrievedEFROG;
        retrievedEFROG = retrievedEFROG.*(sqrt(originalFROG./retrievedFROG));
        retrievedEFROG(bootstrapMask==1) = oldEFr(bootstrapMask==1);
    else
        retrievedEFROG = retrievedEFROG.*(sqrt(originalFROG./retrievedFROG));
    end
    ...
    weightedError = sum(sum((retrievedFROG-originalFROG).^2 .* originalFROG))/sum(sum(originalFROG));
